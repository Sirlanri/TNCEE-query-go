...
func numsChange(ctx iris.Context, db *sql.DB) {
	...
	case "文史":
		...
		//获取三年平均最低折线图
		getExplg, err := db.Prepare(`
			select minscore,minrank,avescore,maxscore from ws17 where name=?
			union
			select minscore,minrank,avescore,maxscore from ws18 where name=?
			union
			select minscore,minrank,avescore,maxscore from ws19 where name=?
		`)
		if err != nil {
			println("预编译表达式出错", err.Error())
		}
